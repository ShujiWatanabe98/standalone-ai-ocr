...
async function validateOcrSheetSet(images){$('#captureMessage').textContent='OCR開始前に用紙の不足・重複・順番を確認しています…';const result=await api('/api/validate-sheet-set',{method:'POST',body:JSON.stringify({imageDataUrls:images})});if(!result.valid)throw new Error(`評価用紙を確認してください：${result.errors.join('／')}`);return result}
...
const startOcrAfterSheetValidation=$('#startButton').onclick;
$('#startButton').onclick=async event=>{const images=[...state.imageDataUrls];if(!images.length)return;const button=$('#startButton');button.disabled=true;try{await validateOcrSheetSet(images);return await startOcrAfterSheetValidation.call(button,event)}catch(error){setProgress(100,'エラー');$('#captureMessage').textContent=error.message;button.disabled=false}};
let backgroundSheetCheckToken=0;
$('#imageInput').addEventListener('change',()=>{const token=++backgroundSheetCheckToken;let attempts=0;const waitForImages=()=>{if(token!==backgroundSheetCheckToken)return;if(state.imageDataUrls.length){const images=[...state.imageDataUrls];$('#captureMessage').textContent='画像を表示しました。バックグラウンドで評価用紙を確認しています…';validateOcrSheetSet(images).then(result=>{if(token!==backgroundSheetCheckToken)return;$('#captureMessage').textContent=result.displayName?`${result.displayName}：ページ構成を確認しました。OCRを開始できます。`:'評価用紙を確認しました。OCRを開始できます。'}).catch(error=>{if(token!==backgroundSheetCheckToken)return;$('#captureMessage').textContent=error.message});return}if(++attempts<120)setTimeout(waitForImages,250)};setTimeout(waitForImages,0)});
