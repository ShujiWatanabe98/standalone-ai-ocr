...
function managementOutcomeDashboard(tenantId) {
  ...
  return {
    ...
    definitions: { severity: '入棟時FIM合計で重症53点以下、中等症54～89点、軽症90点以上に院内集計', homeReturnRate: '分子：退棟日登録済みかつ退院先に自宅・在宅を含む患者。分母：退棟日登録済み患者', fimGain: '患者ごとの最新確定FIM合計－入棟時確定FIM合計の平均' },
  };
}
