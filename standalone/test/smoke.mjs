...
const hospitalPatientResponse = await fetch(`${base}/api/patients`, { method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: hospitalCookie }, body: JSON.stringify({ facilityPatientId: 'HP001', name: '病院テスト患者', birthDate: '1960-01-01' }) });
assert.equal(hospitalPatientResponse.status, 201);
const hospitalPatients = await fetch(`${base}/api/patients`, { headers: { Cookie: hospitalCookie } }).then(r => r.json());
